...
class PublicFunction
{
	...
	{
		add_shortcode('gpsnose_public_index', [$this, 'display_plugin_index_page']);
	}

	/**
	 * Render the index page for this plugin.
	 *
	 * @since 1.0.0
	 */
	public function display_plugin_index_page(mixed $attributes = [], ?string $content = null, ?string $tag = '')
	{
		return $this->renderPage('partials/index.php', $attributes, $content, $tag);
	}

	/**
	 * Returns the content of the requested partial
	 *
	 ...
	 * @return string|null
	 */
	private function renderPage(string $partial, $attributes = [], ?string $content = null, string $tag = ''): ?string
	{
		$record = uniqid('ma_block');
		$gn_data = [];
		$gn_data['Settings'] = [];


		ob_start();
		if (file_exists(__DIR__ . "/" . $partial)) {
			include_once $partial;
		} else {
			echo "<p>partial not found: {$partial} record: {$record}</p>";
		}
